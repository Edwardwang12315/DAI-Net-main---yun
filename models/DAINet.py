...
class Lap_Pyramid_Conv( nn.Module ) :
	...
	def pyramid_decom( self , img ) :
		...
		# 调试点  # print('金字塔分解')  # image = np.transpose( diff[0].detach().cpu().numpy() , (1 , 2 , 0) )  # 调整维度顺序 [C, H, W] → [H, W, C]  # image = (image * 255).astype( np.uint8 )  # plt.imshow( image )  # plt.axis( 'off' )  # plt.show()
...
class DENet( nn.Module ) :
	def __init__( self , num_high = 3 , ch_blocks = 32 , up_ksize = 1 , high_ch = 32 , high_ksize = 3 , ch_mask = 32 ,
		...
		self.KL = DistillKL( T = self.num_high * 2.0 )
		
		self.illum_global = illumination_global( kernel_size = 7 )
		...
		for i in range( 0 , self.num_high ) :
			...
			self.__setattr__( 'down_finetune_layer_{}'.format( i ) , down_finetune( kernel_size = 3 , channels = 3 ) )
	...
	def forward( self , x_dark , x_light ) :
		...
		trans_LF , guide = self.trans_low( trans_LF )
		...
		return HF_d_LF_d , HF_d_LF_l , HF_l_LF_l , HF_l_LF_le
...
class VGG16( nn.Module ) :
	...
	def forward( self , x , Light_dark , Dark_light ) :
		size = x.size()[ 2 : ]
		pal1_sources = list()
		loc_pal1 = list()
		conf_pal1 = list()
		...
		for i in range( 5 ) :
			...
			Dark_light = self.vgg[ i ]( Dark_light )
		
		_Light_dark = Light_dark.flatten( start_dim = 2 ).mean( dim = -1 )
		_Dark_light = Dark_light.flatten( start_dim = 2 ).mean( dim = -1 )
		
		loss_mutual = cfg.WEIGHT.MC * (self.KL( _Light_dark , _Dark_light ) + self.KL( _Dark_light , _Light_dark ))
	...
	def test_forward( self , x_dark , x_light ) :
		with torch.no_grad() :
			...
			loss_mutual = cfg.WEIGHT.MC * (self.KL( _f_DL , _f_LL ) + self.KL( _f_LL , _f_DL ) +  # 高频优化有效
			                               self.KL( _f_DD , _f_DL ) + self.KL( _f_DL , _f_DD ) +  # 暗光增强有效
