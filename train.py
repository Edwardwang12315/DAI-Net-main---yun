...
def train() :
	...
	if args.resume :
		...
		if args.model == 'vgg' or args.model == 'dark' :
			...
			net.backbone.resnet.load_state_dict( base_weights )
	if not args.resume :
		if local_rank == 0 :
			print( 'Initializing weights...' )
		net.backbone.apply( net.weights_init )
	...
	if args.cuda :
		if args.multigpu :
			...
			net = torch.nn.parallel.DistributedDataParallel( net.cuda() ,
			                                                 find_unused_parameters = True )  # net_enh = torch.nn.parallel.DistributedDataParallel(net_enh.cuda())
		# net = net.cuda()
	...
	for epoch in range( start_epoch , cfg.EPOCHES ) :
		...
		for batch_idx , (images , targets , _) in enumerate( train_loader ) :
			...
			# 前向传播两个分支
			t0 = time.time()
			out , loss_mutual , loss_enhance = net( x_dark = img_dark , x_light = images )
			# out = net( img_dark)
